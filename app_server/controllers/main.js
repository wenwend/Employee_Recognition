...
module.exports.postNewEmployee = function(req, res, next) {
    if (req.session.userId && req.session.userType == 'A') {
        const data = req.body;
        //console.log(data)
        client.query('INSERT INTO employee(username, password, admin_id) VALUES ($1,$2,$3);', [data.email, data.pass, req.session.userId], function(err, result) {
            if (err) {
                return next(err);
            }
            //res.send(200);
...
module.exports.postNewAdmin = function(req, res, next) {
    if (req.session.userId && req.session.userType == 'A') {
        ...
        client.query('INSERT INTO admin(email, password, admin_id) VALUES ($1,$2,$3);', [data.email, data.pass, req.session.userId], function(err, result) {
            if (err) {
                return next(err);
            }
            //res.send(200);
...
module.exports.postPassEmployee = function(req, res, next) {
    ...
    if (req.session.userId && req.session.userType == 'E') {
        ...
        client.query("UPDATE employee SET password = ($1) WHERE id = ($2);", [data.pass1, req.session.userId], function(err, result) {
            if (err) {
                return next(err);
            }
            //res.send(200);
...
module.exports.postSignature = function(req, res, next) {
    ...
    if (req.session.userId && req.session.userType == 'E') {
        ...
        client.query('INSERT INTO signature (data, e_id) VALUES ($1, $2) ON CONFLICT (e_id) DO UPDATE SET data = EXCLUDED.data;', [data.file, req.session.userId], function(err, result) {
            if (err) {
                console.log(err);
                return next(err);
            }
...
//updateAdmins
module.exports.editAdmins = function(req, res, next) {
    console.log(req.body);

    if (req.session.userId && req.session.userType == "A" && req.body.id) {
        const data = req.body;
        client.query('UPDATE admin SET email = $2 WHERE id = $1;', [data.id, data.uname], function(err, result) {
            if (err) {
                return next(err);
            }
            //return res.send(200);
...
module.exports.numAwards = function(req, res) {
    if (req.session.userId && req.session.userType == 'A') {
        res.render('numAwards');
    } else {
        res.render('adminLogin', { err: "You must log in as an Admin to access" });
    }
};